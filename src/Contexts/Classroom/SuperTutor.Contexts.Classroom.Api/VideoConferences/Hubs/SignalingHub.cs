...
public class SignalingHub : Hub
{
    ...
    private async Task EmitJoinRoom(string roomName) => await Groups.AddToGroupAsync(Context.ConnectionId, roomName);

    private async Task EmitCreated() => await Clients.Caller.SendAsync("created");

    private async Task EmitJoined(string roomName) => await Clients.Group(roomName).SendAsync("joined");
